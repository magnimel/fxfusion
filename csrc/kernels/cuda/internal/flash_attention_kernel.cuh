...
constexpr int FLASH_BLOCK_SIZE = 16;
...
// Output: out_ctx    {batch, seq, d_model}
//
// Tile-based fused attention with online softmax. Each block handles one
// (head, seq_tile, head_dim_tile) sub-problem. Q, K, V are interleaved
// in qkv at offsets 0, d_model, 2*d_model respectively.
// =====================================================================
__global__ void flash_attention_kernel(
    ...
    for (int64_t k_step = 0; k_step < num_seq_tiles; k_step++) {
        ...
        bool is_fully_masked = (new_max == -INFINITY);

        float scale = is_fully_masked ? 0.0f : expf(running_max - new_max);
        O_acc *= scale;

        float local_sum = 0.0f;
        for (int i = 0; i < FLASH_BLOCK_SIZE; i++) {
            float exp_val = is_fully_masked ? 0.0f : expf(S_s[ty][i] - new_max);
            P_s[ty][i] = exp_val;   
            local_sum += exp_val;
        }
    ...
    if (q_row < seq && out_col < head_dim) {
        ...
        out_ctx[out_idx] = O_acc / running_sum;
    }
}
